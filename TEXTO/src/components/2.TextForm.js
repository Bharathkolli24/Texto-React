...
export default function TextForm(props) {
    ...
    return (
        ...
        <div className="container" style={{color:props.mode ==='light' ?'#232442':'white'}}>
            ...
            <p>{text.split(/\s+/).filter((element)=>{return element.length !== 0}).length} words and {text.length} characters</p>  {/*  if we want bold letters use <b></b>......{return element.length!=0} Here it will say that if the element length != 0 then it'll stay in the array and if = 0 then it'll not stay in the array. Here \s+(backspace s+) says that every white space and new lines count means if we enter space it count as 1 word or if we want to go the new line */}
